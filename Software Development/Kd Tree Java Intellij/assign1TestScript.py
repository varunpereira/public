...
def main():
    ...
    if not os.path.isfile(sExec + ".class"):
        print(sExec + ".java didn't compile successfully.")
    else:
        bCompiled = True
    ...
    if bCompiled:
        ...
        for (j, sInLoopFile) in enumerate(lsInFile):
            ...
            sCommand = "java "+ sExec + " " + sImpl + " \"" + sDataFile + "\" \"" + sInFile + "\" \"" + sOutputFile + "\""
...
def usage(sProg):
    print >> sys.stderr, sProg + " [-v] <code directory> <name of implementation to test> <input data file> <list of test command files>"
    sys.exit(1)
